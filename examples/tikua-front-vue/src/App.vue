...
const startSubscription = async () => {
  if (!provider.value || !walletAddress.value)
...
const drinkPotion = async () => {
  ...
  const tikua = new Tikua({
    ...
    signerAddress: walletAddress.value,
    abi,
  });
...
const checkDragon = async () => {
  ...
  isLoading.value = true;
  const status = await tikua.fetchInspect("dragonStatus", [
    dragonIdInput.value,
  ]);
  message.value = {
    dragon: dragonIdInput.value,
    status: status,
  };
  isLoading.value = false;
};
  ...
  <div class="text-stone-600 dark:text-stone-300 overflow-hidden">
    ...
    <div class="my-16 text-center">
      <button
        v-if="!walletAddress"
        @click="connect"
        class="bg-stone-100 dark:bg-stone-800 rounded-lg p-4 px-6 font-semibold text-xl outline outline-slate-200 dark:outline-slate-600 hover:outline-1 hover:outline-blue-600"
      >
        Connect Wallet
      </button>
      <p v-else>
        Wallet: <span class="text-orange-400">{{ walletAddress }}</span>
      </p>
    </div>
    ...
    >
      <h1
        class="uppercase text-sm font-semibold text-stone-500 dark:text-stone-300"
      >
        Hero
      </h1>
      <div class="flex gap-4">
        <button
          ...
          @click="checkLife"
          class="bg-stone-100 dark:bg-stone-800 rounded-lg p-2 px-4 text-md outline outline-slate-200 dark:outline-slate-600 hover:outline-1 hover:outline-blue-600 disabled:text-gray-300 disabled:outline-none"
        >
          Check Health
        </button>
        <button
    ...
    >
      ...
      <div class="flex gap-4">
        <input
          v-model="dragonIdInput"
          placeholder="Enter a dragon ID"
    ...
    <div
      class="bg-stone-50 dark:bg-stone-700 rounded-lg p-6 my-4 w-[600px] overflow-auto"
    >
